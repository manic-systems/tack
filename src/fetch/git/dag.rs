...
fn filtered_commit_fetch(
    ...
    if let Some(path) = super::local_file_url_path(&parsed_url) {
        ...
        if shallow {
            shallows.insert(want);
        }
        return copy_local_commit_graph(&source, repo, want, haves, shallow);
    }
...
fn copy_local_commit_graph(
    ...
    let have_set = haves.iter().copied().collect::<BTreeSet<_>>();
    let mut seen = BTreeSet::new();
    let mut pending = VecDeque::from([(want, 0_usize)]);
    ...
    while let Some((id, distance)) = pending.pop_front() {
        ...
        if (shallow && distance == 0) || have_set.contains(&id) {
            continue;
        }
