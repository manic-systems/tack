...
impl PinsDoc {
    ...
    pub fn is_recomposable(&self) -> bool {
        self.doc
            .get("tack")
...
impl<'a> FollowAlias<'a> {
    pub fn on_side(self, side: FollowSide) -> Option<&'a str> {
        match self.raw.split_once(':') {
            Some(("flake", rest)) => (side == FollowSide::Flake).then_some(rest),
            Some(("tack", rest)) => (side == FollowSide::Tack).then_some(rest),
            _ => Some(self.raw),
        }
    }

    pub fn flake_side(self) -> Option<&'a str> {
        self.on_side(FollowSide::Flake)
    }
}

pub fn is_global_follow_excluded(
    excludes: &BTreeSet<String>,
    side: FollowSide,
    name: &str,
) -> bool {
    excludes.iter().any(|rule| {
        matches!(
            FollowAlias::from(rule.as_str()).on_side(side),
            Some(part) if part == "*" || part == name
        )
    })
}
...
pub fn global_follow_target<'a>(
    ...
    if is_global_follow_excluded(excludes, side, name) {
        return None;
    }
